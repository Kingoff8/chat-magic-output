...
| `output-template.html` | Структура страницы вывода + рендерер Markdown |
| `prose.css` | Прозаические стили (заголовки, списки, код, каллауты) поверх `demo.html` |
| `render.ps1` | Markdown → HTML; подставляет стили из `demo.html` + `prose.css` |
| `config.txt` | Настройки скилла (`auto_html=on\|off`) |
...
По умолчанию скилл рендерит каждый ответ в HTML **в стиле `demo.html`** (параллакс-фон, стеклянная карточка, «живое» появление блоков) и **открывает его сразу по окончании ответа** — в дополнение к обычному тексту в чате.

`demo.html` — **единственный источник стилей**: `render.ps1` подставляет его `<style>`, а `prose.css` добавляет стили для текста (заголовки, списки, таблицы, код, каллауты).

| Что | Путь |
| --- | --- |
| Исходник ответа | `.chat-magic-output/answer.md` |
| Готовый HTML | `.chat-magic-output/answer.html` |

```powershell
powershell -File render.ps1 -Markdown .chat-magic-output/answer.md `
  -Out .chat-magic-output/answer.html -Theme opencode
```

- Палитра берётся из `palette.txt`.
- Папка `.chat-magic-output/` одноразовая — в git не попадает.
- Отключить: `config.txt` → `auto_html=off`.
