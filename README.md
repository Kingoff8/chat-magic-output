...
| `output-template.html` | Шаблон HTML-вывода: рендерер Markdown + 7 палитр |
| `render.ps1` | Markdown → стилизованный HTML |
...
По умолчанию скилл рендерит каждый ответ в стилизованный HTML **в стиле `demo.html`** (параллакс-фон, стеклянная карточка, «живое» появление блоков) и **открывает его сразу по окончании ответа** — в дополнение к обычному тексту в чате.
